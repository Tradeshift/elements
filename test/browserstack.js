...
const checkReport = function(report) {
	...
	if (!report.length) {
		...
		console.log(
			'Check the tests runs! https://travis-ci.org/Tradeshift/elements/pull_requests'
		);
		fail();
		return false;
	}
	...
	report.forEach(function(browserRes) {
		...
		out.push(chalk`{white.bgBlack Browser: } {white.bold.bgBlack ${browserRes.browser}}`);
		if (browserRes.tests && browserRes.tests.length) {
			browserRes.tests.forEach(function(test) {
				...
				if (test.status === 'failed') {
					...
					errOut.push(chalk`Browser: {red.bold ${browserRes.browsr}`);
					...
					out.push(chalk`
						{green ${test.suiteName} > ${test.name}} ${timeString}
					`);
				}
			});
		} else {
